...
export function GitHubWidget({ id, onRemove, onSettings }: { id: string; onRemove: (id: string) => void; onSettings: (id: string) => void }) {
          ...
          <div className="flex flex-col items-center">
            <div className="flex items-center gap-2">
              <Badge variant="outline" className="bg-green-100 text-green-800">
                {data.stats.mergedPRs} merged
              </Badge>
              <Badge variant="outline" className="bg-yellow-100 text-yellow-800">
                {data.stats.openPRs} open
              </Badge>
            </div>
            <span className="text-sm text-gray-500 mt-1">Pull Requests</span>
          </div>
        </div>
      </div>
    </DashboardWidget>
