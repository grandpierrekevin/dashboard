...
export function SonarQubeGraph() {
  ...
  const activity = days.map(day => ({
    date: day,
    issues: projects.reduce((sum, project) => {
  ...
  return (
    <div className="w-full bg-[#181C23] rounded-xl p-6 shadow-lg mb-8">
      <div className="flex items-center justify-between mb-2">
        <h2 className="text-2xl font-bold">Activité SonarQube sur 7 jours</h2>
        <select
        ...
        <ResponsiveContainer width="100%" height="100%">
          {chartType === 'line' ? (
            <LineChart data={activity}>
              ...
              <RechartsTooltip />
              <Line type="monotone" dataKey="issues" stroke="#22c55e" name="Issues" strokeWidth={2} />
            </LineChart>
          ) : (
            <BarChart data={activity}>
              ...
              <YAxis allowDecimals={false} />
              <RechartsTooltip />
              <Bar dataKey="issues" fill="#22c55e" name="Issues" />
            </BarChart>
          )}
        </ResponsiveContainer>
      </div>
