...
import { SonarData } from "@/types/mocks";
...
export const SonarWidget = memo(function SonarWidget({ widgetId }: SonarWidgetProps) {
  const { data, loading, error } = useWidgetData({ type: 'sonarqube' });
  const { settings, updateSetting } = useChartSettings('sonarqube');
  ...
  const sonarData = Array.isArray(data) ? data : [];
  const chartData: ChartData[] = sonarData.map(item => ({
    name: item.name,
    value: item.stars || 0
  }));

  const totalStars = sonarData.reduce((sum, item) => sum + (item.stars || 0), 0);
  ...
  return (
    <Card>
      ...
      <CardContent>
        ...
        <WidgetDetails data={chartData} title="SonarQube Activity" type="sonarqube" />
      </CardContent>
    </Card>
  );
});
